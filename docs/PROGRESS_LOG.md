...
> **分支:** `feat/ml-platform`
> **最後更新:** 2026-06-27 15:41
> **狀態:** M1 計算核心就緒（特徵 → 訓練 → 持久化），待 4.2/4.3
> **測試總計:** 33 passed / 覆蓋率 93%
...
| 里程碑 | 目標 | 狀態 |
|---|---|---|
| M1 核心管線可訓練 | 一鍵訓練 + 超參數 + 模型儲存 | 🔄 計算核心完成（4.2/4.3 未做） |
...
| `src/ml/jobs.py` | 38 | 19 | 50% | 任務 2.2 產物，4.2 將補滿 |
| **TOTAL** | **300** | **21** | **93%** | 門檻 80% ✅ |
...
| 順序 | 任務 | 依賴 | 說明 |
|---|---|---|---|
| 1 | **4.2 非同步 job** | 4.1 ✅ | 把 train_model + save_model 包進 jobs.JobStore 背景執行，補滿 jobs.py 覆蓋 |
| 2 | 4.3 訓練 API | 4.2 | POST /api/ml/train、GET /train/status/{job_id} |
| 3 | 5.2 模型清單 + active 切換 | 5.1 ✅ | 完成後預測端可取 active 模型 |
| 4 | 6.x 單筆 / CSV 批次預測 | 5.x | M2 |

**尚未提交:** 本 log 檔（`docs/PROGRESS_LOG.md`）。
