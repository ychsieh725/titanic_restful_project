...
**建立日期:** 2026-06-27
**最後更新:** 2026-06-27（3.1 特徵工程管線 進行中）
...
| 3.1 | 特徵工程管線（補值/Title/FamilySize/編碼，可序列化） | 🔄 進行中 | 高 | 2.2 | 3h | FR-2.1~2.4, 2.7（共用管線） |
| 3.2 | 管線單元測試（RED→GREEN） | ⏳ 待處理 | 高 | 3.1 | 1.5h | testing 80%+ |
| 4.1 | 訓練服務：LR+RF, GridSearchCV cv≥5, train/test split, 指標 | ⏳ 待處理 | 高 | 3.1 | 3h | FR-3.2~3.6 |
...
| 5.1 | 模型持久化（joblib 整條管線）+ metadata 寫入 ml_model | ⏳ 待處理 | 高 | 4.1, 2.3 | 2h | FR-4.1, 4.2 |
...
| 里程碑 | 目標 | 包含任務 | 狀態 |
|--------|------|----------|------|
| M1: 核心管線可訓練 | 一鍵訓練 + 超參數 + 模型儲存 | 2.x, 3.x, 4.x, 5.1 | ⏳ 待處理 |
