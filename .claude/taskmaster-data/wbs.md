...
**建立日期:** 2026-06-27
**最後更新:** 2026-06-28（8.x 完成 → M3 達成；MVP 全部必做項通過，待開 PR 合回 main）
...
| 6.2 | CSV 批次預測 + 結果 CSV 下載 | ✅ 完成 | 高 | 6.1 | 2h | FR-5.3, 5.4 |
| 6.3 | 輸入驗證（schema-based，明確錯誤訊息，不回 500） | ✅ 完成 | 高 | 6.1 | 1.5h | FR-5.5, NFR-S1/S2 |
| 7.1 | 訓練頁（演算法選擇 + job 輪詢狀態） | ✅ 完成 | 高 | 4.3 | 2h | FR-7.4 |
| 7.2 | 模型管理頁（清單 + 設 active） | ✅ 完成 | 中 | 5.2 | 1.5h | FR-4.3 |
| 7.3 | 預測頁（單筆表單 + CSV 上傳結果表格） | ✅ 完成 | 高 | 6.2 | 2.5h | FR-5.1, 5.3 |
| 7.4 | 導覽列整合（首頁/訓練/模型/預測） | ✅ 完成 | 中 | 7.1 | 1h | FR-7.1, 7.2 |
| 8.1 | 整合測試（API 端點 + DB） | ✅ 完成 | 高 | 6.3, 5.2 | 2h | 端到端流程 test_acceptance.py |
| 8.2 | 驗收檢查（6 大必做項逐項驗證） | ✅ 完成 | 高 | 7.3, 8.1 | 1h | §8 六大必做項逐項綠燈 |
...
| M2: 預測可用 | 單筆 + CSV 批次預測（含機率） | 5.2, 6.x | ✅ 完成 |
| M3: MVP 驗收 | UI 串接 + 6 大必做驗收通過 | 7.x, 8.x | ✅ 完成 |
